...
func TestCdxHelper(t *testing.T) {
	...
	tests := []struct {
		...
		{"MCP login uses callback input", []string{"mcp", "login", "posthog"}, []string{"mcp", "login", "posthog"}, 0},
		{"MCP login keeps account and options", []string{"--account", "team", "mcp", "login", "posthog", "--timeout", "10m", "--scopes", "openid,user:read"}, []string{"mcp", "login", "posthog", "--timeout", "10m", "--scopes", "openid,user:read", "--account", "team"}, 0},
		{"MCP login keeps empty account for validation", []string{"--account", "", "mcp", "login", "posthog"}, []string{"mcp", "login", "posthog", "--account", ""}, 0},
		{"MCP login help", []string{"mcp", "login", "--help"}, []string{"mcp", "login", "--help"}, 0},
		{"MCP login exit code", []string{"mcp", "login", "posthog"}, []string{"mcp", "login", "posthog"}, 17},
		{"MCP list stays native", []string{"mcp", "list", "--json"}, append(defaults, "mcp", "list", "--json"), 0},
		{"MCP logout stays native", []string{"mcp", "logout", "posthog"}, append(defaults, "mcp", "logout", "posthog"), 0},
