...
~~~sh
git tag -a v0.6.0 -m 'Codator v0.6.0'
test "$(git rev-parse HEAD)" = "$(git rev-list -n1 v0.6.0)"
...
~~~sh
gh release create v0.6.0 --repo olafurns7/codator --draft --verify-tag \
  --title 'Codator v0.6.0' \
...
gh workflow run release.yml --repo olafurns7/codator --ref v0.6.0 \
...
gh release download v0.6.0 --repo olafurns7/codator --dir "$verify_dir" --pattern SHA256SUMS
gh release download v0.6.0 --repo olafurns7/codator --dir "$verify_dir" --pattern attestations.jsonl
...
gh attestation verify "$verify_dir/SHA256SUMS" \
  ...
  --cert-identity "https://github.com/olafurns7/codator/.github/workflows/release.yml@refs/tags/v0.6.0" \
...
~~~sh
gh release edit v0.6.0 --repo olafurns7/codator --draft=false
~~~
